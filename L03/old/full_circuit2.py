...
#range index can return keys in a range
class RangeIndex(object):
  ...
  def list(self, first_key, last_key):
    ...
    return [key for key in self.data if first_key <= key <= last_key] #this is useful
...
class BlitRangeIndex(object):
  ...
  def add(self, key):
    ...
    self.data.insert(self._binary_search(key), key)#array insert method index , key
...
class AvlRangeIndex(object):
  ...
  def __init__(self):
    """Initially empty range index."""
    self.tree = RangeTree() #instance without new call in py
...
class BSTNode(object):
  ...
  def find(self, key):
    ...
    if key < self.key:
      ...
      return self.right and self.right.find(key) # and same as javascript? take the second
    return self
...
class AVL(BST):# python inherit like this AVL inherit BST
