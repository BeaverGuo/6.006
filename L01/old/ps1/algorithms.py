...
def algorithm1(problem, trace = None):
    ...
    (subStartR, subNumR) = (0, problem.numRow)#行不变，根据列分成2个子问题subNumC1,subNumC2
    (subStartC1, subNumC1) = (0, mid)
    (subStartC2, subNumC2) = (mid + 1, problem.numCol - (mid + 1))

    subproblems = []
    subproblems.append((subStartR, subStartC1, subNumR, subNumC1)) #分成C1,C2两个子问题,存入subproblems[]里面
...
def algorithm4(problem, bestSeen = None, rowSplit = True, trace = None):
    ...
    newBest = sub.getLocationInSelf(problem, bestSeen)
    if not trace is None: trace.setProblemDimensions(sub)
    result = algorithm4(sub, newBest, not rowSplit, trace)# rowSplit取反了
    return problem.getLocationInSelf(sub, result)
