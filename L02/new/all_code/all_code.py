...
#       gets a 5 then a 2, we keep track of (18, 72). SO we want the total number of credits, and then the weighted sum of the total number of credits.
#       Then, the student's
#       GPA is 72/18=4.
#
#   TODO:
...
class Gradebook:
    ...
    def update_grade(self, student, credit, grade):
      ...
      #  self.upperStudents.show_tree()
        ...
        if student in self.lowerStudents.key_to_index:
            ...
            # if the maximum of the lower quartile is more than the minimum of
            # the k_array
            if self.lowerStudents.data[0] > min_k:
                ...
                max_k = self.all_students_scores[
                    self.middleStudentNames[k - 1]][2]
                ...
                if max_k > self.upperStudents.data[0]:
                    ...
                    # should be inserte dinot the minmimum heap.
                    self.upperStudents.insert_key(max_k_student_name, max_k)
            ...
            if self.upperStudents.data[0] < max_k:
                ...
                # k_array and middleStudents length should be the same.
                max_k_student_name = self.middleStudentNames[k - 1]
                former_min_student_name = self.upperStudents.keys[0]
                self.middleStudentNames[k - 1] = former_min_student_name
          ...
          #      ###print("swapping"  + k_student_name+" with "+ former_max_student_name)
                ...
                self.lowerStudents.insert_key(
                    k_student_name, min_k)  # insert_key
        else:
            #print("student is ")
            # print(student)
            #print("gpa is ")
    ...
    def middle(self):
        ...
        for index in range(0, len(self.middleStudentNames)):
            # get the keys that match up with that index.
            student_name = self.middleStudentNames[index]
         #   ###print("the gpa")
...
#                                "Bob Dylan": 1,
#                                "Bob Marley": 2}
#       where 0, 1, 2 corresponds to the index in "self.keys" #so the sself.keys respond to the index in the self.keys index.
#
#   Provided Methods:
...
#
# so you want to sort the keys, and then sort the data,but in the self.key_to_index you want to simply change the indices where
# the keys are lined up to and not the order of the names or tuples.
#               self.keys = ["Bob Marley", "Bob Dylan", "Ray Charles"]
#               self.data = [5, 1, 4]  #so this is a max heap
#       self.key_to_index = {"Ray Charles": 2,
#                           "Bob Dylan": 1,
#                           "Bob Marley: 0"}
# ARE STUDENT GPAs discrete.
###################################################################
#heap数据结构特点:父节点的值一定不大于(Min_Heap)或不小于(Max_Heap)子节点的值
...
class Max_Heap:
    ...
    def __init__(self, keys, data):
        try:
            ...
            self.keys = collections.deque(keys)#https://docs.python.org/2/library/collections.html
            self.data = collections.deque(data)
            self.key_to_index = dict(
                zip(self.keys, range(len(self.keys))))  #给keys增加有序的索引 !
            self.heapify()
        except Exception as e:
            pass

    # TODO 就是在heap树里修改一个key的data并保持heap树的结构没被破坏
    ...
    def increase_key(self, i, key):#往parent找,插入key到对应的位置
        ...
        self.data[i] = key
        parent = (i - 1) / 2
        # yyou want to get the self = self.data is more than kelf.data  of the
        # parents. SO
        while i > 0 and self.data[i] > self.data[parent]:
            # I  is the current key of the =actualy key, but the index oc that.
            self.swap(i, parent)
            i = parent
            parent = (i - 1) / 2
    ...
    def swap(self, i1, i2):
        self.key_to_index[self.keys[i1]] = i2  #  交换index!
        self.key_to_index[self.keys[i2]] = i1  # !
        self.data[i1], self.data[i2] = self.data[i2], self.data[i1]#交换data
        self.keys[i1], self.keys[i2] = self.keys[i2], self.keys[i1]#交换key
