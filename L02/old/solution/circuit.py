...
class PriorityQueue:
    ...
    def append(self, key):#二叉树特点比顶点小的都在左边，反之在其右边
    ...
    def min(self):#顶点是最小的?那heap[0]是不是None?
        """Returns the smallest element in the queue."""
        return self.heap[1]
    
    def pop(self):#最小值直接把第一个heap[1]返回即可,但树的结构会变化需要处理
        ...
        if len(heap) == 2:
            return heap.pop()
        heap[1] = key = heap.pop()#第一个没了，最后一个补上
        
        i = 1
        while True:#二分遍历找最小的,再替换heap[1]
            left = i * 2
            if len(heap) <= left:
                break
            left_key = heap[left]
