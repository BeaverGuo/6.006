...
class TruthTable:
    ...
    def output(self, inputs):#inputs是2的位数，表示输入的对数，2个一对?
    ...
    def _build_table(self, output_list):#output_list应该是一个输入列表,将output_list无限细分数组直到只有2个元素
        # Builds an evaluation table out of a list of truth table values.
        #
        # Raises:
        #    TypeError: An exception if the list's length is not a power of two.估计没有非门
    ...
    def _table_depth(self, table):#log2(N)
...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        self.min_index = None#添加新项最小index重置
    
    def min(self):
        """The smallest element in the queue."""
        if len(self.queue) == 0:#代码应对可能产生的输入，一般是不好的
            return None
        self._find_min()
        return self.queue[self.min_index]
    
    def pop(self):#优先队列pop移掉最小的那项
    ...
    def _find_min(self):
        ...
        min = self.queue[0]
        self.min_index = 0
        for i in range(1, len(self.queue)):#可以看出是一个O(n)算法复杂度
            key = self.queue[i]
            if key < min:
                min = key
                self.min_index = i
